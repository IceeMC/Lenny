...
class TicTacToe {
    ...
    removePlayer(user) {
        if (this.players.get(user.id)) {
            ...
            this.msg.send(":x: **You not in this game!**");
        }
    }
    ...
    async drawBoard() {
        try {
            const slotBuffers = [];
            for (const slot of this.board) slotBuffers.push(this._getTaker(slot));
            const mainBoard = await fs.readFileSync(join(__dirname, "..", "assets", "ttt.png"));
            const blankSlot = Buffer.alloc(1);

            const newBoard = new Canvas(500, 500)
                .addImage(mainBoard, 0, 0, 500, 500)
                .addImage(this.board[0].taken ? slotBuffers[0] : blankSlot, 0, 0, 164, 164)
                .addImage(this.board[1].taken ? slotBuffers[1] : blankSlot, 169, 0, 164, 164)
                .addImage(this.board[2].taken ? slotBuffers[2] : blankSlot, 169 * 2, 0, 164, 164)
                .addImage(this.board[3].taken ? slotBuffers[3] : blankSlot, 0, 169, 164, 164)
                .addImage(this.board[4].taken ? slotBuffers[4] : blankSlot, 169, 169, 164, 164)
                .addImage(this.board[5].taken ? slotBuffers[5] : blankSlot, 169 * 2, 169, 164, 164)
                .addImage(this.board[6].taken ? slotBuffers[6] : blankSlot, 0, 169 * 2, 164, 164)
                .addImage(this.board[7].taken ? slotBuffers[7] : blankSlot, 169, 169 * 2, 164, 164)
                .addImage(this.board[8].taken ? slotBuffers[8] : blankSlot, 169 * 2, 169 * 2, 164, 164);
            const board = await newBoard.toBufferAsync();
            await this.msg.channel.send(new MessageAttachment(board));
