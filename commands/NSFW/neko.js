...
class Neko extends Command {
    ...
    constructor(...args) {
        super(...args, {
            ...
            cooldown: 5,
            aliases: ["inappropriateneko", "lewdneko"]
        });
    }

    async run(message) {
        if (!message.channel.nsfw) throw message.language.get("NSFW_CHANNEL_NO_NSFW");
        const url = await this.client.utils.p0rn("nsfw_neko_gif");
        ...
        embed.setImage(url);
        embed.setTimestamp();
        embed.setFooter(`Neko requested by: ${message.author.tag} | NSFW provided by: nikos.life`);
